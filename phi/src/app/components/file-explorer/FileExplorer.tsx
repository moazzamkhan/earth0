      ...
      {!readonly && (
        <div className="btn-group btn-group-sm">
          <button
            ...
            onClick={() => onDeleteItem(currentThing.id)}
          >
          ...
          things.map((t: Thing) => (
            <a
              ...
              onClick={(e: SyntheticEvent) =>
                onItemClick((e.currentTarget as HTMLAnchorElement).getAttribute("id"))
              }
              className={`list-group-item list-group-item-action ${
                currentThing.id === t.id ? "active" : ""
              }`}
            >
  ...
  return {
    ...
    onNewItem: (type: string) => {
      type &&
        dispatch(
          addThing(
            ThingUtils.createThingInstance(
              type,
              `${type}#${Generator.numlt100()}`,
              null
            )
          )
        )
